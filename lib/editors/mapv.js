...
import * as TagList from './../helpers/tag-list';

export async function load(buffer : EndianIo) {
	// load taglist
	const tag_list = await TagList.load('C:\\Games\\Halo Online\\assembly\\TagNames\\tagnames_1.106708 cert_ms23.csv')
	...
	for(let i = 0; i < 0x280; i++) {
		...
		object.type = buffer.readInt8().toString(16);
	...
	for(let i = 0; i < 0x100; i++) {
		...
		const tag_index = buffer.readInt32();
		tag_entry.tag = tag_list.filter(val => { return val.ident == tag_index })[0] || {
			ident: tag_index,
			name: null
		};
